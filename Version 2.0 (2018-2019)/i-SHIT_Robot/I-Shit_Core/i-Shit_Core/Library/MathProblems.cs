...
namespace i_Shit_Core.Library {
    public class MathPloblems {
        ...
        /// <returns></returns>
        public static TriPoint twistCompute (float X, float Z) {
            ...
            //return resTwist;
            TriPoint resTwist = new TriPoint ();
            //resTwist.X = -1 + 1 * Z;
            Z = 1;
            ...
            //角度左正右负
            angel = Math.Atan (X / Z);
            Console.WriteLine ("TwistComputer: Angle: " + angel);
            if (Math.Abs (angel) > 0.1745) //当偏角大于10 度时，转动
            {
                resTwist.Z = angel * 0.25;
        ...
        /// <param name="rPos"></param>
        /// <returns></returns>
        public static TriPoint twistComputeColorPoint (ColorSpacePoint rPos) {
            TriPoint resTwist = new TriPoint ();
            resTwist.X = 0.12;
            resTwist.Y = 0;
            if (rPos.X < -50) {
        ...
        private static TriPoint CorrentSpeed = new TriPoint (0, 0, 0);
        private static float datea = 0.03f;
        /// <summary>
        /// 平滑速度处理
        /// </summary>
        /// <param name="twist">目标速度</param>
        public static TriPoint smoothSpeed (TriPoint twist) {
            TriPoint detaSpeed = new TriPoint (twist.X - CorrentSpeed.X, twist.Y - CorrentSpeed.Y, twist.Z - CorrentSpeed.Z);
            //处理X方向速度
