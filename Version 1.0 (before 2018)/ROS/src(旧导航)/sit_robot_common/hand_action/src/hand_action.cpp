...
char begin4[8] = {0x77,0x4B,0x87,0xAF,0x9B,0xB8,0x54,0x88 };
char begin5[8] = {0x77,0x4B,0x87,0x87,0x5F,0xB8,0x54,0x88 };
...
//****串口初始化
  char dev[]  = "/dev/handodommove"; //USB串口
  fd = OpenDev(dev);
  tcflush(fd, TCIFLUSH);
  set_speed(fd,115200);
...
//case HAND_TYPE_CATCH_TWO_STEPS
		...
		cout << "begin5" << endl;	write(fd, begin5, LENGTH);	sleep(2);
		...
		cout << "撤回手臂0" << endl;	write(fd, begin5, LENGTH);	sleep(2);
