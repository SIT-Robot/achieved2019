...
void VelocitySmoother::reconfigCB(yocs_velocity_smoother::paramsConfig &config,
		uint32_t level) {
	ROS_INFO("Reconfigure request : %f %f %f %f %f", config.speed_lim_v,
			config.speed_lim_w, config.accel_lim_v, config.accel_lim_w,
			config.decel_factor);
...
void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr& msg) {
	...
	rev_vel.linear.x = msg->linear.x;
	rev_vel.linear.y = msg->linear.y;
	rev_vel.angular.z = msg->angular.z;

	// Bound speed with the maximum values
	target_vel.linear.x =
			msg->linear.x > 0.0 ?
					std::min(msg->linear.x, speed_lim_v) :
					std::max(msg->linear.x, -speed_lim_v);
	target_vel.linear.y =
			msg->linear.y > 0.0 ?
					std::min(msg->linear.y, speed_lim_v) :
					std::max(msg->linear.y, -speed_lim_v);
	...
	while (!shutdown_req && ros::ok()) {
		...
		if ((robot_feedback != NONE) && (input_active == true)
				...
				&& (((ros::Time::now() - last_cb_time).toSec()
						> 5.0 * cb_avg_time)
						|| // 5 missing msgs
						(std::abs(current_vel.linear.x - last_cmd_vel.linear.x)
								> 0.2)
						|| (std::abs(
			...
			ROS_WARN(
					"Using robot velocity feedback (%s) instead of last command: %f, %f, %f",
		...
		ROS_INFO("tar:::%f", target_vel.linear.y);
		ROS_INFO("last:::%f", last_cmd_vel.linear.y);
		...
		if ((target_vel.linear.x != last_cmd_vel.linear.x)
			...
			double v_inc_x, v_inc_y;
			double max_v_inc_x, max_v_inc_y;
...
//      }
			...
			if ((robot_feedback == ODOMETRY)
				...
				// countermarch (on robots with significant inertia; requires odometry feedback to be detected)
				max_v_inc_x = decel_lim_v * period;
			} else {
				max_v_inc_x = (
						(v_inc_x * target_vel.linear.x > 0.0) ?
								accel_lim_v : decel_lim_v) * period; //decel_lim_v  = decel_factor*accel_lim_v;
			}

			if ((robot_feedback == ODOMETRY)
					&& (current_vel.linear.y * target_vel.linear.y < 0.0)) {
				// countermarch (on robots with significant inertia; requires odometry feedback to be detected)
				max_v_inc_y = decel_lim_v * period;
			} else {
				max_v_inc_y = (
						(v_inc_y * target_vel.linear.y > 0.0) ?
								accel_lim_v : decel_lim_v) * period;
			}

//      v_inc = sqrt(v_inc_x*v_inc_x+v_inc_y*v_inc_y);
			v_inc = v_inc_x;

			max_v_inc = max_v_inc_x;

//			max_v_inc = sqrt(
//					max_v_inc_x * max_v_inc_x + max_v_inc_y * max_v_inc_y);

			ROS_INFO("aaaaaaaaaaa%f", v_inc);

			ROS_INFO("bbbbbbbbbb%f", max_v_inc);
			...
			if ((robot_feedback == ODOMETRY)
				...
				max_w_inc = (
						(w_inc * target_vel.angular.z > 0.0) ?
								accel_lim_w : decel_lim_w) * period;
			}
			...
			if (rev_vel.linear.y != 0) {

				ROS_INFO("aaaaaaaaaaayyyyy%f", v_inc_y);

				ROS_INFO("bbbbbbbbbbyyyyy%f", max_v_inc_y);
				double MA = sqrt(v_inc_y * v_inc_y + w_inc * w_inc);
				double MB = sqrt(max_v_inc_y * max_v_inc_y + max_w_inc * max_w_inc);

				double Av = std::abs(v_inc_y) / MA;
				double Aw = std::abs(w_inc) / MA;
				double Bv = max_v_inc_y / MB;
				double Bw = max_w_inc / MB;
				double theta = atan2(Bw, Bv) - atan2(Aw, Av);

				if (theta < 0) {
					// overconstrain linear velocity
					max_v_inc_y = (max_w_inc * std::abs(v_inc_y)) / std::abs(w_inc);
				}
			...
			ROS_INFO("%f k%f k%f", cmd_vel->linear.x, cmd_vel->linear.y,
					cmd_vel->angular.z);
...
bool VelocitySmoother::init(ros::NodeHandle& nh) {
	// Dynamic Reconfigure
	dynamic_reconfigure_callback = boost::bind(&VelocitySmoother::reconfigCB,
			this, _1, _2);

	dynamic_reconfigure_server = new dynamic_reconfigure::Server<
			yocs_velocity_smoother::paramsConfig>(nh);
	...
	odometry_sub = nh.subscribe("odometry", 1, &VelocitySmoother::odometryCB,
			this);
	current_vel_sub = nh.subscribe("robot_cmd_vel", 1,
			&VelocitySmoother::robotVelCB, this);
	raw_in_vel_sub = nh.subscribe("raw_cmd_vel", 1,
			&VelocitySmoother::velocityCB, this);
...
public:
	...
	virtual void onInit() {
		...
		NODELET_DEBUG_STREAM(
				"Velocity Smoother : initialising nodelet...[" << name << "]");
		vel_smoother_.reset(new VelocitySmoother(name));
		if (vel_smoother_->init(ph)) {
			NODELET_DEBUG_STREAM(
					"Velocity Smoother : nodelet initialised [" << name << "]");
			worker_thread_.start(&VelocitySmoother::spin, *vel_smoother_);
		} else {
			NODELET_ERROR_STREAM(
					"Velocity Smoother : nodelet initialisation failed [" << name << "]");
		}
	}
