...
float robotWheelL = 0.21;//大轮子到中心的距离
...
//	float L = 0.071;
	...
	nV1=4300.0f * (cos(Vth) * xx + sin(Vth) * yy + robotWheelL*Vth); //修改4300.0f可改变轮子速度比例
	nV2=4300.0f * (-sin(PI/3.0f+Vth) * yy - cos(PI/3.0f+Vth) * xx + robotWheelL*Vth);
	nV3=4300.0f * ( sin(PI/3.0f-Vth) * yy - cos(PI/3.0f-Vth) * xx + robotWheelL*Vth);
