...
int Odom3=0;
/****TEST****/
float O1=0;//lunzi licheng
float O2=0;
float O3=0;

//float OdomHz=0.02;//机器人里程计采样频率

double sV1 = 0;//机器人大轮子速度（m/s） 
double sV2 = 0;
double sV3 = 0;	
//***************************************
...
void setTermios(struct termios * pNewtio, int uBaudRate)
{
	...
	pNewtio->c_oflag = 0;
	pNewtio->c_lflag = ICANON; //non ICANON
...
int main(int argc, char** argv){
  ...
  struct timeval tv;
  const char *dev ="/dev/handodommove";
  fd_set rfds;
  	...
  	if (select(1+fd, &rfds, NULL, NULL, &tv)>0||1)
  	{
  		...
  		if (FD_ISSET(fd, &rfds)||1)
  		{
 			...
 			if(odoHeadFlag==1 && odoEndFlag==1)
  			{	
				sV1 = Odom1 * PI * robotOdoD / 4000/0.1;
				sV2 = Odom2 * PI * robotOdoD / 4000/0.1;
				sV3 = Odom3 * PI * robotOdoD / 4000/0.1;
				
				O1 += sV1;  O2 += sV2; O3 += sV3;
				//cout<<"INPUT-Odom------>Odom1:::"<<O1 <<"  Odom2:::"<<O2<<"  Odom3:::"<<O3<<endl; 
