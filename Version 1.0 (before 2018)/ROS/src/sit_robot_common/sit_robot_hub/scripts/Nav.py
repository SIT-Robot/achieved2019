...
class Nav(object):
    def __init__(self, current_pose=PoseStamped(), peoples=PositionMeasurementArray(),
                 face=FaceRecognitionActionFeedback()):
        self.current_pose = current_pose
        self.velpub = rospy.Publisher('/base_cmd_vel', Twist, queue_size=1)
        self.peoples = peoples
        self.facepub = rospy.Publisher('/fr_order', FRClientGoal, queue_size=1)
        self.face = face
    ...
    def PeopleNum(self,lenth):
        self.getpeople()
        num = 0
        for peo in self.peoples.people:
            if abs(peo.pos.x)<lenth:
                num = num + 1
        print num
        return str(num)
    ...
    def facerecognitionTrain(self, order, args):
        fr = FRClientGoal()
        fr.order_id = order
        fr.order_argument = args
        self.facepub.publish(fr)

    def FaceCallBack(self, data=FaceRecognitionActionFeedback()):
        self.face = data

    def facerecognition(self):

        rospy.Subscriber('/face_recognition/feedback', FaceRecognitionActionFeedback, self.FaceCallBack)
        rate = rospy.Rate(1.0)
        rate.sleep()
        print self.face.feedback.names
        if len(self.face.feedback.names) is 0:
            return ""
        return self.face.feedback.names[0]
    ...
    def readdata(self, data):
        ...
        elif "PeopleNum" in list:
            if len(list) is not 2:
                return 0
            return self.PeopleNum(list[1])
        elif "FaceRecognitionTrain" in list:
            if len(list) is not 3:
                return 0
            return self.facerecognitionTrain(int(list[1]), list[2])
        elif "FaceRecognition" in list:
            if len(list) is not 1:
                return 0
            return self.facerecognition()
        else:
